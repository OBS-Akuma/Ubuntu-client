/**
 * Adds room preset management functionality to the Kirka game client.
 * Allows saving, loading, and organizing room settings presets.
 */
const roomPresetsAddon = () => {
  'use strict';
  ...
  function getMapNameFromSettings(modal) {
  ...
  function snapshotSettings(modal) {
    const data = {};
  ...
  function applySettings(modal, data) {
    modal.querySelectorAll(".wrapper-input.select .input").forEach((input) => {
      const label = input.closest(".element")?.querySelector(".label");
      if (!label) return;
      const key = label.firstChild?.textContent?.trim();
  ...
  function getPresetNameFromData(settings, index) {
  ...
  function createButton(text, onClick, type = 'blue') {
  ...
  function createOverButton(onClick) {
    const label = document.createElement('label');
    label.className = 'custom-checkbox checkbox-size';
    label.setAttribute('data-v-730c0c40', '');
    label.setAttribute('data-v-5d0fefa8', '');
  ...
  function renderList(modal, container) {
    ...
    presets.forEach((preset, i) => {
      ...
      item.innerHTML = `
        <span class="kp-drag" draggable="true" style="
          ...
          letter-spacing: 1px;
        ">⋮⋮</span>
        <input class="kp-name" type="text" value="${escapeHtml(displayName)}" spellcheck="false" style="
          flex: 1;
...
module.exports = { roomPresetsAddon };